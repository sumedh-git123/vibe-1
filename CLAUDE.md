...
- Revision 3 (user: sharper and smoother film, better transitions, bigger headers, bold high-contrast problem, more interaction):
  - The film was upscaled with Higgsfield `upscale_video` (bytedance, 2k, 60fps, preset aigc; 0.48 credits). The source is `review/hero-2k60.mp4`. Frames were re-extracted at full quality (user: "quality means everything"): `film/d` has **168 frames at the full 2554x1440 (28fps, WebP q78, 15 MB)**; `film/m` has 80 frames at 1280x720 (q74, 3 MB). The canvas renders at up to 2x device pixel ratio with high-quality smoothing. Keep d+m+other files at or under 255 files so the artifact preview can publish (currently 251). The canvas cross-blends neighbouring frames (float frame index), so scrubbing is smooth.
  ...
  - Problem section: "Six cents on the dollar. One missed count takes it." (second line red) and a bolded lede. The Today/With BidMate lists were replaced by the **bid-day widget** (`[data-bidday]`): a Counted by hand / With BidMate switch, four example missed items ($21,600 on a $1.2M job with a $72,000 profit), per-item Approve/Undo (**With BidMate opens fully approved and green**; Undo puts a line back in review, and Approve all reappears), and an animated profit number and bar. Labeled as example numbers.
